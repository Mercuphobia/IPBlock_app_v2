...
void add_ipset_have_mac_to_chain(const char *ipset_name, const char *mac) {
    snprintf(command, sizeof(command), "iptables -S BLOCK_IP_CHAIN_HAVE_MAC | grep -q -- '-m mac --mac-source %s -m set --match-set %s dst -j DROP'", mac, ipset_name);
    if (system(command) != 0) {
        snprintf(command, sizeof(command), "iptables -A BLOCK_IP_CHAIN_HAVE_MAC -m mac --mac-source %s -m set --match-set %s dst -j DROP", mac, ipset_name);
        system(command);
...
void delete_ipset_have_mac_to_chain(const char *ipset_name, const char *mac) {
    snprintf(command, sizeof(command), "iptables -S BLOCK_IP_CHAIN_HAVE_MAC | grep -q -- '-m mac --mac-source %s -m set --match-set %s dst -j DROP'", mac, ipset_name);
    if (system(command) == 0) {
        snprintf(command, sizeof(command), "iptables -D BLOCK_IP_CHAIN_HAVE_MAC -m mac --mac-source %s -m set --match-set %s dst -j DROP", mac, ipset_name);
        system(command);
            ...
            if(start_block_time < end_block_time){
                if(local_time < start_block_time || local_time > end_block_time){
                    ...
                    if(strcmp(filename,list[i].url) == 0){
                        ...
                        add_ipset_have_mac_to_chain(ipset_name, list[i].mac);
